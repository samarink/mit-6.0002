...
class Patient(object):
    ...
    def update(self):
        ...
        survived_bacteria = [
            bacteria for bacteria in self.bacteria if not bacteria.is_killed()]
        population_density = len(survived_bacteria) / self.max_pop

        offsprings = []

        for bacteria in survived_bacteria:
            try:
                offspring = bacteria.reproduce(population_density)
...
def calc_pop_avg(populations, n):
    ...
    """
    bacteria_population = sum(populations[i][n]
...
def simulation_without_antibiotic(num_bacteria,
    ...
    bacteria_array = np.array(total_bacteria_population)
    bacteria_mean = bacteria_array.mean(axis=0)
    timesteps = np.arange(1, 301)

    make_one_curve_plot(timesteps, bacteria_mean, 'Timestep',
                        'Average Population', 'Without Anitbiotic')
...
def calc_pop_std(populations, t):
    ...
    for trial in range(len(populations)):
        standart_deviation += (populations[trial][t] - mean)**2

    return math.sqrt(1 / len(populations) * standart_deviation)
...
def calc_95_ci(populations, t):
    ...
    standart_deviation = calc_pop_std(populations, t)
    sem = standart_deviation / math.sqrt(len(populations))
    width = 1.96 * sem
    ...
    def reproduce(self, pop_density):
        ...
        if random.random() <= reproduction_probability:
            if self.get_resistant():
                ...
                is_resistant = random.random() <= resistance_probability

                return ResistantBacteria(self.birth_prob, self.death_prob, is_resistant, self.mut_prob)
...
class TreatedPatient(Patient):
    ...
    def get_resist_pop(self):
        ...
        return len([bacteria for bacteria in self.bacteria if bacteria.get_resistant()])
...
def simulation_with_antibiotic(num_bacteria,
    ...
    for _ in range(num_trials):
        ...
        for _ in range(num_bacteria):
            bacteria_list.append(ResistantBacteria(
                birth_prob, death_prob, resistant, mut_prob))

        patient = TreatedPatient(bacteria_list, max_pop)

        bacteria_population = []
        resistant_bacteria_popopulation = []

        for _ in range(150):
            population = patient.get_total_pop()
            bacteria_population.append(population)
            resistant_population = patient.get_resist_pop()
            resistant_bacteria_popopulation.append(resistant_population)
        ...
        total_bacteria_population.append(bacteria_population)
        total_resistant_population.append(resistant_bacteria_popopulation)

    total_array = np.array(total_bacteria_population)
    resistance_array = np.array(total_resistant_population)
    bacteria_mean = total_array.mean(axis=0)
    resistance_mean = resistance_array.mean(axis=0)
    timesteps = np.arange(1, 401)

    make_two_curve_plot(timesteps, bacteria_mean, resistance_mean, 'Total', 'Resistant', 'Timestep',
