...
test = importlib.util.spec_from_file_location("test", "test.pyc")
...
class ps3_P1A(unittest.TestCase):
    ...
    def test_is_tile_cleaned_dirty(self):
        ...
        for x, y in xyrange(width, height):
            self.assertFalse(room.is_tile_cleaned(x, y),
                             "Unclean tile {} was returned as clean".format(
                                 (x, y))
                             )
    ...
    def test_clean_tile_at_position_PosToZero(self):
        ...
        for x, y in xyrange(width, height):
            room.clean_tile_at_position(test.Position(
                x + random.random(), y + random.random()), dirt_amount)
    ...
    def test_clean_tile_at_position_PosToPos(self):
        ...
        for x, y in xyrange(width, height):
            room.clean_tile_at_position(test.Position(
                x + random.random(), y + random.random()), dirt_amount - 1)
            # using random.random in case there is any issue with specific parts of a tile
        for x, y in xyrange(width, height):
            self.assertFalse(room.is_tile_cleaned(x, y),
    ...
    def test_clean_tile_at_position_ZeroToZero(self):
        ...
        for x, y in xyrange(width, height):
            room.clean_tile_at_position(test.Position(
                x + random.random(), y + random.random()), 1)
            # using random.random in case there is any issue with specific parts of a tile
        for x, y in xyrange(width, height):
            self.assertTrue(room.is_tile_cleaned(x, y),
                            "Clean tile {} was marked clean, no negative dirt allowed".format(
                                (x, y))
                            )
    ...
    def test_get_num_cleaned_tiles_FullIn1(self):
        ...
        for x, y in xyrange(width-1, height-1):
            ...
            self.assertEqual(num_cleaned, cleaned_tiles,
                             "Number of clean tiles is incorrect: expected {}, got {}".format(
                                 cleaned_tiles, num_cleaned)
                             )
            ...
            for y in [0.0, -0.1, height - 0.1, height, height + 0.1]:
                ...
                self.assertEquals(solution_room.is_position_in_room(pos), room.is_position_in_room(pos),
                                  "position {},{} is incorrect: expected {}, got {}".format(
                                      x, y, solution_room.is_position_in_room(pos), room.is_position_in_room(pos))
                                  )
...
class ps3_P2_ER(unittest.TestCase):
    ...
    def test_get_random_position(self):
        ...
        for i in range(50000):
            ...
            try:
                ...
                self.fail(
                    "get_random_position returned {} which is not a Position".format(pos))
    ...
    def test_get_num_tiles(self):
        ...
        for i in range(10):
            width, height, dirt_amount = (
                random.randint(1, 10), random.randint(1, 10), 1)
            room_num_tiles = ps3.EmptyRoom(
                width, height, dirt_amount).get_num_tiles()
            sol_room_tiles = ps3.EmptyRoom(
                width, height, dirt_amount).get_num_tiles()
...
class ps3_P2_FR(unittest.TestCase):
    ...
    def test_is_tile_furnished(self):
        ...
        for trial in range(5):
            width, height, dirt_amount = (
                random.randint(2, 8), random.randint(2, 8), 1)
        ...
        for m in range(7000):
            ...
            for r in robots:
                ...
                try:
                    ...
                    self.fail(
                        "get_robot_position returned %r which is not a Position" % (p,))
...
class ps3_P5_Standard(SimulationTester):
    ...
    def testSimulation1(self):
        ...
        try:
            self.run_simulation(
                ((142, 173),), (1, 1.0, 1, 5, 5, 1, 1.0, 100, ps3.StandardRobot))
        except:
    ...
    def testSimulation2(self):
        ...
        self.run_simulation(((183, 198),), (1, 1.0, 1, 10,
                                            10, 1, 0.75, 100, ps3.StandardRobot))

    def testSimulation3(self):
        "Test cleaning 90% of a 10x10 room (Standard Robot)"
        self.run_simulation(((298, 327),), (1, 1.0, 1, 10,
                                            10, 1, 0.9, 100, ps3.StandardRobot))

    def testSimulation4(self):
        "Test multiple robots (95% of a 20x20 room with 5 robots (Standard Robot))"
        self.run_simulation(((289, 303),), (5, 1.0, 1, 20,
                                            20, 1, 0.95, 100, ps3.StandardRobot))

    def testSimulation5(self):
        "Test different speeds (90% of a 5x20 room with a robot of speed 0.2 (Standard Robot))"
        self.run_simulation(((1095, 1228),), (1, 0.2, 1, 5,
                                              20, 1, 0.9, 100, ps3.StandardRobot))
    ...
    def testSimulation9(self):
        ...
        self.run_simulation(((387, 447), (384, 444)),
                            (1, 0.2, 2, 3, 10, 6, 0.9, 100, ps3.StandardRobot))

    def testSimulation10(self):
        "Test multiple robots (95% of a 10x10 room with 5 robots (Standard Robot)) capacity = 2, 6 dirt/tile"
        self.run_simulation(((137, 198), (130, 190)),
                            (5, 1.0, 2, 10, 10, 6, 0.95, 100, ps3.StandardRobot))
    ...
    def testSimulation11(self):
        ...
        self.run_simulation(((48, 108), (45, 104)),
                            (3, 0.5, 2, 5, 5, 6, 0.9, 100, ps3.StandardRobot))
...
class ps3_P5_Faulty(SimulationTester):
    ...
    def testSimulation2(self):
        ...
        x = ps3.run_simulation(1, 1.0, 1, 10, 10, 1,
                               0.75, 100, ps3.FaultyRobot)
        self.assertTrue(
            210 <= x <= 260, "Simulation output was outside of 99.7% confidence interval! Took " + str(x) + " steps\n")

    def testSimulation3(self):
        "Test cleaning 90% of a 10x10 room with FaultyRobot"
        x = ps3.run_simulation(1, 1.0, 1, 10, 10, 1, 0.9, 100, ps3.FaultyRobot)
        self.assertTrue(
            360 <= x <= 415, "Simulation output was outside of 99.7% confidence interval! Took " + str(x) + " steps\n")

    def testSimulation4(self):
        "Test cleaning 100% of a 5x5 room with FaultyRobot"
        x = ps3.run_simulation(2, 1.0, 2, 5, 5, 5, 1.0, 100, ps3.FaultyRobot)
        self.assertTrue(
            160 <= x <= 200, "Simulation output was outside of 99.7% confidence interval! Took " + str(x) + " steps\n")

    def testSimulation5(self):
        "Test cleaning 75% of a 10x10 room with FaultyRobot"
        x = ps3.run_simulation(4, 1.0, 3, 10, 10, 5,
                               0.75, 100, ps3.FaultyRobot)
        self.assertTrue(
            101 <= x <= 116, "Simulation output was outside of 99.7% confidence interval! Took " + str(x) + " steps\n")

    def testSimulation6(self):
        "Test cleaning 90% of a 10x10 room with FaultyRobot"
        x = ps3.run_simulation(5, 1.0, 3, 10, 10, 10,
                               0.9, 100, ps3.FaultyRobot)
        self.assertTrue(
            205 <= x <= 225, "Simulation output was outside of 99.7% confidence interval! Took " + str(x) + " steps\n")
